...
const CollectionForm: React.FC<CollectionFormProps> = ({ onClose, onSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-end sm:items-center justify-center z-[100] p-0 sm:p-4 backdrop-blur-sm">
      <div className="bg-white rounded-t-[5px] sm:rounded-[5px] w-full max-w-md max-h-[min(92dvh,92vh)] flex flex-col overflow-hidden shadow-2xl border-t sm:border border-gray-100 mb-0 sm:mb-0">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col flex-1 min-h-0">
          <div className="flex-1 overflow-y-auto overscroll-contain p-6 space-y-5">
            <div className="space-y-1.5">
              ...
              />
              {errors.title && (
                <p className="mt-1 text-[10px] font-bold text-red-600 uppercase ml-1">{errors.title.message}</p>
              )}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              />
              {errors.amount && (
                <p className="mt-1 text-[10px] font-bold text-red-600 uppercase ml-1">{errors.amount.message}</p>
              )}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              />
              {errors.description && (
                <p className="mt-1 text-[10px] font-bold text-red-600 uppercase ml-1">{errors.description.message}</p>
              )}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              />
              {errors.dueDate && (
                <p className="mt-1 text-[10px] font-bold text-red-600 uppercase ml-1">{errors.dueDate.message}</p>
              )}
            </div>
          </div>

          <div className="flex gap-3 p-4 border-t border-gray-100 bg-white shrink-0 pb-[max(1rem,env(safe-area-inset-bottom))]">
            <Button
              type="button"
              variant="secondary"
